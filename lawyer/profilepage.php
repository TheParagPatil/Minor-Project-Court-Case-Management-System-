...
$stmt = $con->prepare("SELECT lawyer_first_name, lawyer_last_name, lawyer_email, lawyer_phone_no, lawyer_city, lawyer_address, lawyer_rating, specialization, lawyer_image from lawyer_login WHERE lawyer_id = ?");
...
if (isset($_POST['img-submit'])) {
	...
	// $stmt = $conpdo->prepare("UPDATE lawyer_login SET lawyer_image = ? WHERE lawyer_id = ?");
	// $stmt->bindParam(1, $img_data);
	// $stmt->bindParam(2, $_SESSION['lawyer_id']);
	// $stmt->execute();
    print_r($_FILES);
    if($_FILES['f1']['name']){
        print_r($_FILES);
        $target ="C:/xampp/htdocs/latest/Minor/Minor/lawyer/image/".$_FILES['f1']['name'];
        move_uploaded_file($_FILES['f1']['tmp_name'], $target);
        $img="http://000webhost.com/image/".$_FILES['f1']['name'];
        }
        $i = "UPDATE lawyer_login
        SET lawyer_image = '$img'
        WHERE lawyer_id = '$lid' "; 
        // $i="insert into reg(image)values('$img')";
        ...
        <div class="row">
            ...
            <div class="col-sm-10">
                ...
                <div class="profile_info">
                    <?php
echo '<form action="" method="post" enctype="multipart/form-data"><input type="file" name="f1"><input type="submit" name="img-submit"></form>';
